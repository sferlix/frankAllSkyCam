...
basic_comando = fileManager.getCameraBinary() + " -n -o " + image_file + " --gain 18  --ev 0 --awbgains 1.5,2  --roi 0.25,0.25,0.5,0.5 --width " + width +" --height " + height + " --immediate "
...
@lru_cache(maxsize=1)
def _model0():
    return numpy.poly1d(numpy.polyfit(MOD0_SQRT, MOD0_SQM, 5))

@lru_cache(maxsize=1)
def _model005():
    return numpy.poly1d(numpy.polyfit(MOD005_SQRT, MOD005_SQM, 3))

@lru_cache(maxsize=1)
def _model01():
    return numpy.poly1d(numpy.polyfit(MOD01_SQRT, MOD01_SQM, 5))

@lru_cache(maxsize=1)
def _model5():
    return numpy.poly1d(numpy.polyfit(MOD5_SQRT, MOD5_SQM, 6))
...
def readSQM(daytime=False):
    ...
    if SQM_LE == "y":
       ...
       sqm = getRealSQM(SQM_LE_IP,SQM_LE_PORT)
       if sqm < 0:
          sqm_le = "n"
...
def calculateRMS(image_file):
    print("Calculating SQRT RMS...1")
    temp=Image.open(image_file)
    print("Calculating SQRT RMS...2")
    stat = ImageStat.Stat(temp)
    print("Calculating SQRT RMS...3")
    r,g,b = stat.rms
    print("Calculating SQRT RMS...4")
    sqrt_rms = math.sqrt(0.241*(r**2) + 0.691*(g**2) + 0.068*(b**2))
    print("Calculating SQRT RMS...5")
...
def getRealSQM(ip, port):
    ...
    for i in range(max_retries):
        try:
            # timeout=3 assicura che non rimanga bloccato se il device non risponde
