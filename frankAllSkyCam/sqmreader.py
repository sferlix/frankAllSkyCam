...
sqmFolder = appPath + str(config['system']['sqmFolder'])
outputFolder = appPath + str(config['system']['otuputFolder'])

log_file = sqmFolder +"/log_sqm.txt"
image_file = outputFolder + "/sqm_temp.jpg"
data_file = sqmFolder + "/data.csv"

basic_comando = "libcamera-still -n -o " + image_file + " --gain 18  --ev 0 --awbgains 1.5,2  --roi 0.25,0.25,0.5,0.5 --width 800 --height 600 --immediate "
...
def logInfo(secs, rms, model, psqm, sqm):
     ...
     with open(data_file, 'a') as f:
          a = str(secs)+","+ str(round(rms,2)) + ","+ str(sqm) + "\n"
          f.write(a)
          f.close()
     return
